...
public class ConcluidasActivity extends AppCompatActivity {
    ...
    public void obterOrdens() {
        Call<ListaOrdem> call = FiftyoneApplication.getInstance().getServiceOrdem().listarOrdensCompletadas();

        call.enqueue(new Callback<ListaOrdem>() {
            @Override
            public void onResponse(Call<ListaOrdem> call, Response<ListaOrdem> response) {
                OrdemAdapter adapter = new OrdemAdapter(response.body().getLista(), ConcluidasActivity.this);
                lista.setAdapter(adapter);
            }

            @Override
            public void onFailure(Call<ListaOrdem> call, Throwable t) {
                Toast.makeText(ConcluidasActivity.this, "Ops", Toast.LENGTH_LONG).show();
            }
        });
    }
}
